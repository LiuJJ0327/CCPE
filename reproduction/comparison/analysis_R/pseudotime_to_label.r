rm(list = ls())
library(mclust)
CCPE_pseudotime<-read.csv("./pseudotime_analysis/3_output/CCPE_pseudotime.csv", header = F)
...
write.table(CCPE_predict_label,file="./comparison/1_mesc_Quartz/output/predlabel/CCPE_predlabel.txt",row.names = F,col.names = F,quote = F)
